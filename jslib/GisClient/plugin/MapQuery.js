Ext.define('GisClient.plugin.MapQuery', {
	requires: ['Ext.toolbar.Spacer', 'Ext.button.Split'],
    mixins: {
        observable: 'Ext.util.Observable'
    },
	
    alias : 'plugin.gc_mapquery',
    alternateClassName : 'GisClient.MapQuery',
    ...
    init: function(mapPanel){
		...
		var activeIndex = 0;
		var queryTools = Ext.create('Ext.button.Split', {
			enableToggle: true,
		...
		queryTools.menu.items.each(function(item, index) {
			item.on({checkchange: function(item, checked) {
				...
				if(checked) {
					...
					queryTools.setIconCls(item.iconCls);
					queryTools.setTooltip(item.text);
				}
			}});
		});
		
		mapPanel.getDockedComponent('tbTop').add([" ","-"," ",queryTools]);
		mapPanel.actions = actions;
		
		this.queryTools = queryTools;
		//Inizializza l'icona giusta
		queryTools.setIconCls(queryTools.menu.getComponent(self.activeToolIndex).iconCls);
		mapPanel.resultLayer = this.resultLayer;
	...
	initSelActions: function(){
		...
		/*
		new GeoExt.Action({
			text: this.queryTooltip,
			iconCls: 'info',
			map: self.mapPanel.map,
			control: new OpenLayers.Control.GetFeature({
                protocol: OpenLayers.Protocol.WFS.fromWMSLayer(this.queryLayers[0]),
                box: true,
                hover: true,
	...
	setQueryLayers: function(){
		...
		switch (this.selgroupActive) { 
			...
			case 'ActiveLayer': 
				...
				if(this.activeLayer){
					console.log('Attivo: ' + this.activeLayer.name);
...
//console.log(this.queryLayers);
	...
	getFeatures: function(e){
		...
		if(this.nquery == 0){
			...
			this.mapPanel.fireEvent('loadqueryresult',{start:true});
		}
		...
		var options = {
            ...
            callback: function(response) {
				...
				if(features.length>0){
					...
					this.mapPanel.fireEvent('loadqueryresult',{layer:layer, featureType:featureType, features:features});
				}
				if(this.nquery == this.nresponse){
					this.nquery = this.nresponse = 0;
					this.mapPanel.fireEvent('loadqueryresult',{end:true});
