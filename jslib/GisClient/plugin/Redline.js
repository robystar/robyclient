Ext.define('GisClient.plugin.Redline', {
    ...
    alias : 'plugin.gc_redline',
    alternateClassName : 'GisClient.Redline',
	...
	serviceURL: '/gisclient/services/redline.php',
    ...
    init: function(mapPanel){
		...
		actions['redline-line'] = Ext.create('GeoExt.Action', {
		...
		actions['redline-label'] = Ext.create('GeoExt.Action', {
		...
		var label = Ext.create('Ext.Toolbar.TextItem', {
			id:'labelRedline',
			xtype: 'tbtext',
			text: this.labelText,
			style: {
		...
		var activeIndex = 0;
		var redLineTools = Ext.create('Ext.button.Split', {
			iconCls: "drawfree",
			tooltip: "redline",
			enableToggle: true,
			...
			menu: new Ext.menu.Menu({
				items: [
					Ext.create('Ext.menu.CheckItem', actions["redline-line"]),
					Ext.create('Ext.menu.CheckItem', actions["redline-label"]),
					'-',
					{
						id: 'save-session',
		...
		redLineTools.menu.items.each(function(item, index) {
			item.on({checkchange: function(item, checked) {
				...
				if(checked) {
					...
					redLineTools.setIconCls(item.iconCls);
				}
			}});
		});
		
		mapPanel.getDockedComponent('tbTop').add([" ","-"," ",redLineTools]);
	...
	addVectorLayer: function(){
		...
		this.redlineVectLayer.events.on({
            "featureadded": function(e){
				...
				Ext.getCmp('save-session').enable();
				Ext.getCmp('delete-session').enable();				
